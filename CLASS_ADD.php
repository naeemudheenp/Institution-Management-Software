...
if(isset($_POST['fclass'])&& isset($_POST['fyear']) && isset($_POST['fteacher'])){
  ...
  $result_set=mysql_fetch_array($RESULT_CHECK);
	if($result_set){
		die("This Class Is Already Taken! Try Other");
	}
  ...
  $result_set=mysql_fetch_array($RESULT_CHECK);
	...
	$SQL="INSERT INTO class VALUES('".$HOST_ID."','".$HOST_NAME."','".$HOST_YEAR."','".$HOST_TEACHER."')";
	$SQL_UPDATE=mysql_query($SQL,$SERVER_CONN);
	if(!$SQL_UPDATE){

		die("Unable TO Regseter");

	}
	else{
		$SQL="UPDATE users SET class='".$HOST_NAME."' WHERE username='".$HOST_TEACHER."'";
		$SQL_UPDATE=mysql_query($SQL,$SERVER_CONN);
		if(!$SQL_UPDATE){die ("UNABLE TO ADD");}
		echo("<h1><center>CLASS REGISTERED<bR>");
    echo("<h4><center>CLASS NAME:'".$HOST_NAME."'</h4><bR>");
